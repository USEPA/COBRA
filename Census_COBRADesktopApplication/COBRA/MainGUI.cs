...
{ "_PM_Acute_Myocardial_Infarction_Nonfatal","Nonfatal Heart Attacks" },
    ...
    "___O3_School_Loss_Days"};
        ...
        public MainGUI()
        {
            ...
            //comboBox_DAC.SelectedIndexChanged += ComboBox_DAC_SelectedIndexChanged;
            ...
            map2.AddLayer("shp/cb_2023_us_tract_5m.shp");
        ...
        private void LoadDACDropdown()
        {
            //comboBox_DAC.Items.Clear();

            // Add the "All Counties" item
            //comboBox_DAC.Items.Add("(No Filter)");
            //comboBox_DAC.SelectedIndex = 0; // Set "All Counties" as the default selection
        ...
        /*private void ComboBox_DAC_SelectedIndexChanged(object sender, EventArgs e)
        {
            // This code will be executed whenever the selected index (selected item) of comboBox_DAC changes.
            calculateSummaryVals();
            SetResultsContent();
        }*/
        ...
        private void calculateSummaryVals()
        {
            ...
            foreach (SYS_ResultsRow result in this.cobraDataSet.SYS_Results.Rows)
            {
                ...
                {
                    //skip adding benefits if county doesnt match selected county
                ...
                //skip adding benefits if filtering on CJEST but not a CJEST population
                //if (String.Equals(comboBox_DAC.SelectedItem.ToString(), "CJEST") && (result.IsCJESTNull() || result.CJEST != 1)) continue;

                double IRAfrac = 1;
                //skip adding benefits if filtering on IRA but not an IRA population
                /*if (String.Equals(comboBox_DAC.SelectedItem.ToString(), "IRA"))
                {
                    if (!result.IsIRANull() && result.IRA != 0)
                    {
                        IRAfrac = result.IRA; // If the IRA is a nonzero value, use it
                    }
                    else
                    {
                        continue; // Skip if IRA is null or zero
                    }
                }*/
                ...
                curLow += (result._PM_Mortality_All_Cause__low_ * IRAfrac);
                ...
                curHigh += (result._PM_Mortality_All_Cause__high_) * IRAfrac;
                ...
                curLowVal += (result.___PM_Mortality_All_Cause__low_ * IRAfrac);
                ...
                curHighVal += (result.___PM_Mortality_All_Cause__high_ * IRAfrac);
                ...
                foreach (string colval in outputCols)
                {
                    ...
                    else if (colval.Contains("_O3_"))
                    {
                        ...
                        curVal += ((double)propertyValue * IRAfrac);
        ...
        private void SetResultsContent()
        {
            ...
            /*if (comboBox_DAC.SelectedItem != null && comboBox_DAC.SelectedItem.ToString() != "(No Filter)")
            {
                string community = comboBox_DAC.SelectedItem.ToString();
                selection = $"{community} communities in {selection}";
            }*/
   ...
   $@"\row";
        ...
        private void tabControlCombinedEmissions_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (tabControlCombinedEmissions.SelectedIndex == 1)
            {
                ...
                SQLiteConnection dbConn = new SQLiteConnection(Globals.connectionstring);
                dbConn.Open();
                AQProcessor processor = new AQProcessor(dbConn, Globals.connectionstring);
        ...
        private void comboBox_emissionsfieldselect_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            Console.WriteLine("IN MAP LAYER COUNT FUNCTION: ********************************************************************* " + AppDomain.CurrentDomain.BaseDirectory);
            DataTable dt = null;
            if (map1.Layers.Count > 0)
            {
                Console.WriteLine("MAP1 LAYERS COUNT IS");
                Console.WriteLine(map1.Layers.Count);
                MapPolygonLayer countyLayer = default(MapPolygonLayer);

                countyLayer = (MapPolygonLayer)map1.Layers[0];
                Console.WriteLine("Layer Feature Count: " + countyLayer.DataSet.Features.Count);
        ...
        private void btnExecute_Click(object sender, EventArgs e)
        {
            ...
            {
                // check if the user is using custom emissions or not
                usePreloadedEmissionsData = radioButton_data1.Checked
            };
        ...
        {
            Console.WriteLine("****************** IN MAP FUNCTION ************");
            DataTable dt = null;
            if (map2.Layers.Count > 0)
            {
                MapPolygonLayer tractLayer = default(MapPolygonLayer);

                tractLayer = (MapPolygonLayer)map2.Layers[0];
                ...
                else
                {
                    ...
                    lookup.Columns["TOTALPMORTLOW"].Expression = "[PM Mortality All Cause (low)] + [PM Infant Mortality] + [O3 Mortality Longterm exposure] + [O3 Mortality Shortterm exposure]";
                    lookup.Columns["TOTALPMORTLOWVAL"].Expression = "[PM Mortality All Cause (low)] + [PM Infant Mortality] + [O3 Mortality Longterm exposure] + [O3 Mortality Shortterm exposure]";

                    lookup.Columns["TOTALPMORTHIGH"].Expression = "[PM Mortality All Cause (high)] + [PM Infant Mortality]";
                    lookup.Columns["TOTALPMORTHIGHVAL"].Expression = "[$ PM Mortality All Cause (high)] + [$ PM Infant Mortality]";
                    lookup.Columns["TOTALPMORTLOW"].Expression = "[PM Mortality All Cause (low)] + [PM Infant Mortality]";
                    lookup.Columns["TOTALPMORTLOWVAL"].Expression = "[$ PM Mortality All Cause (low)] + [$ PM Infant Mortality]";
                    ...
                    //Get the shapefile's attribute table to our datatable dt
                    dt = tractLayer.DataSet.DataTable;

                    //make sure Value exists in the shapefile layer and that it's treated like a double
                    if (dt.Columns.Contains("Value"))
                    {
                        Console.WriteLine("Recreating 'Value' column with double type...");
                        // Save existing data if needed
                        List<double> existingValues = dt.Rows.Cast<DataRow>()
                            .Select(row =>
                                double.TryParse(row["Value"]?.ToString(), out double result) ? result : 0.0)
                            .ToList();
                        ...
                        {
                            dt.Rows[i]["Value"] = existingValues[i];
                        }
                    }
                    else
                    {
                        Console.WriteLine("Creating 'Value' column with double type...");
                        dt.Columns.Add("Value", typeof(double));
                    }

                    //assign Values from COBRA results to tractLayer
                    for (int row = 0; row < dt.Rows.Count; row++)
                    {
                        try
                        {
                            string tract_id = dt.Rows[row]["GEOID"].ToString();
                            DataRow[] foundRows = lookup.Select("dest_tract='" + tract_id + "'");
                            dt.Rows[row]["Value"] = 0;
                            ...
                            if (foundRows.Length > 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        {
                                            case "PMHAALLRESP":
                                                value2set = Convert.ToDouble(foundRows[0]["PM HA All Respiratory"]) +
                                                            Convert.ToDouble(foundRows[0]["PM HA Respiratory2"]);
                                                dt.Rows[row]["Value"] = value2set;
                                                break;
                                            case "ALLASTHMA":
                                                value2set = Convert.ToDouble(foundRows[0]["Asthma Exacerbation Cough"]) +
                                                            Convert.ToDouble(foundRows[0]["Asthma Exacerbation Shortness of Breath"]) +
                                                            Convert.ToDouble(foundRows[0]["Asthma Exacerbation Wheeze"]) +
                                                            Convert.ToDouble(foundRows[0]["Asthma Exacerbation Chest Tightness"]);
                                                dt.Rows[row]["Value"] = value2set;
                                                break;
                                            default:
                                                double defaultValue = Convert.ToDouble(foundRows[0][field2name]);
                                                dt.Rows[row]["Value"] = defaultValue;
                                                break;
                                        }
                                    }
                                    catch (Exception ex)
                                    {
                                        Console.WriteLine($"Error processing value for field '{field2name}': {ex.Message}");
                                    }
                                }
                            }
                        }
                    ...
                    // need to make sure all values are actually doubles are the categories on the legend wont work
                    for (int row = 0; row < dt.Rows.Count; row++)
                    {
                        {
                            if (dt.Rows[row]["Value"] == DBNull.Value || dt.Rows[row]["Value"] == null)
                            {
                                Console.WriteLine($"Row {row} has a null Value. Defaulting to 0.");
                                dt.Rows[row]["Value"] = 0.0; // Default to zero
                            }
                            else
                            {
                                // Attempt to convert to double to ensure consistency
                                try
                                {
                                    dt.Rows[row]["Value"] = Convert.ToDouble(dt.Rows[row]["Value"]);
                                }
                                catch (Exception ex)
                                {
                                    Console.WriteLine($"Error converting Value to double for row {row}: {ex.Message}");
                                    dt.Rows[row]["Value"] = 0.0; // Default to zero
                                }
                            }
                        }
                    }
                    Console.WriteLine($"Column 'Value' DataType AFTER CORRECTION: {dt.Columns["Value"].DataType}");
                    ...
                    scheme.EditorSettings.UseGradient = false;
                    scheme.EditorSettings.FieldName = "Value";
                    if (!dt.Columns.Contains(scheme.EditorSettings.FieldName))
                    {
                        Console.WriteLine($"Error: Field '{scheme.EditorSettings.FieldName}' does not exist in DataTable.");
                    }


                    scheme.CreateCategories(tractLayer.DataSet.DataTable);
                    var categores = scheme.Categories;
                    //this foreach loop will fail if Value column isn't set up right - causing categories to not be set up right
                    foreach (var category in scheme.Categories)
                    {
                        try
                        {
                            Console.WriteLine($"Category: Min = {category.Range.Minimum}, Max = {category.Range.Maximum}");
                        }
                        catch (Exception ex)
                        {
                            Console.WriteLine($"FAILED TO SET UP MAP COLOR CATEGORIES: {ex.Message}");
                        }
                    }
                    tractLayer.Symbology = scheme;
                    tractLayer.LegendText = "US Census Tracts - " + comboBox3.Text;
    ...
    "$ O3 School Loss Days, All Cause"
                        ...
                        //write the header line but replace dest_tract with 'Census Tract ID'
                        sw.WriteLine(string.Join(",", colnames.Select(col => col == "dest_tract" ? "\"Census Tract ID\"" : $"\"{col}\"")));
                        ...
                        foreach (SYS_ResultsRow result in this.cobraDataSet.SYS_Results.Rows)
                        {
                          ...
                          //now write the totals we want the first 5 values to be empty because we dont care to sum the fips, ID, dest_tract state, county cols, cjest, ira ccols
                        var formattedTotals = totals.Select((total, index) => index < 7 ? "\"\"" : $"\"Total: {total}\"");
    ...
    "$ O3 School Loss Days, All Cause"
                    ...
                    {
                        if (colname == "dest_tract") worksheet.Cell(1, colcount).Value = "Census Tract ID";
                        else worksheet.Cell(1, colcount).Value = colname;
                        colcount++;
                    ...
                    int columnNumber = 8;
                    var cellWithFormulaA1 = worksheet.Cell(2, 8);
                    #region Header_Totals
                    for (int i = 7; i < colnames.Length; i += 1)
                    {
                    ...
                    //totals with dollar formatting

                    columnNumber = 14;

                    //starting with the 12th header val(colnames) aka Total health benefits
                    cellWithFormulaA1 = worksheet.Cell(2, 14);
                    for (int i = 13; i < colnames.Length; i += 2)
                    {
                        cellWithFormulaA1 = worksheet.Cell(2, columnNumber);
                        string columnLetter = GetExcelColumnLetter(columnNumber);
                        cellWithFormulaA1.FormulaA1 = string.Format("=\"Total: \"&TEXT(ROUND(SUM({0}3:{0}3110),2),\"#,###,##0.00\")", columnLetter);
                    ...
                    #endregion
                    var headerTotals = worksheet.Range("F2:CM2");
                    ...
                    var rngData = worksheet.Range("F2:CM3111");
                    ...
                    var footerTotals = worksheet.Range("F3111:CM3111");
