...
# Creating an Installer for the Desktop Application (.exe)
1. Delete the bin folders for both COBRA and cobra_console
2. Build the solution in Release mode (make sure top of visual stuid project shows Release x64)
...
9. Final installer with proper version should be put in cobra/installer/cobrainstaller folder
10. COBRA installer has historically been put in H drive ERD/COBRA/COBRA.NET/installer/ (should see a bunch of installers in this folder)
...
in cobra console + click 'run custom tool' on the cobradataset.xsd
...
# Ensuring efficiency for the Census-Level SR Matrix (SYS_SR_PM)
- you must make sure that if it is changed for any reason that the following indices are created:
    CREATE INDEX idx_sourceindx ON "SYS_SR_PM"(sourceindx);
    CREATE INDEX idx_censusindx ON "SYS_SR_PM"(census_index);
    CREATE INDEX idx_typeindx ON "SYS_SR_PM"(typeindx);
- failing to index this table will result in over 40x slower performance
