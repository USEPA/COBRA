...
export class EmissionspanelComponent implements OnInit {
  ...
  emitFromEmissionspanelToReviewpanel(data: any) {
    this.lastUsedLocations = data.statetree_items_selected;
  ...
  public selectLastUsedLocations(codes: string[]) {
    //codes is a list of either state fips codes or county fips codes
    // get state codes (2 chars)
    const stateCodes = codes.filter((code) => code.length == 2);
    const countyCodes = codes.filter((code) => code.length == 5);
    const countiesInState = new Set(countyCodes.map((code) => code.substring(0, 2)));
    for (var i = 0; i < this.state_clr_structure.length; i++) {
      if (stateCodes.includes(this.state_clr_structure[i].STFIPS)) {
        this.state_clr_structure[i].selected = ClrSelectedState.SELECTED;
        continue;
      }
      // decide whether we need to loop through counties
      if (countiesInState.has(this.state_clr_structure[i].STFIPS)) {
        this.state_clr_structure[i].selected = ClrSelectedState.INDETERMINATE;
        this.state_clr_structure[i].expanded = true;
        for (var j = 0; j < this.state_clr_structure[i].counties.length; j++) {
          if (countyCodes.includes(this.state_clr_structure[i].counties[j].FIPS)) {
