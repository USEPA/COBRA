...
calculateChanges(data): {"NOx": number, "PM2.5": number, "SO2": number, "NH3": number, "VOC": number} {
    const changes = {"NOx": 0, "PM2.5": 0, "SO2": 0, "NH3": 0, "VOC": 0};
    const updatePacket = data.updatePacket;

    // Iterate over each pollutant in pollutantsList
    data.pollutantsList.forEach(pollutant => {
        const name = pollutant.name;
        const reduceIncrease = pollutant.reduce_increase;
        const value = pollutant.value ? parseFloat(pollutant.value) : 0;
        const percentTons = pollutant.percent_tons;
        
        // Ensure currentAmount is defined and valid; if not, set it to 0
        const currentAmount = updatePacket[name.replace('PM2.5', 'PM25')];
        if (currentAmount == null || isNaN(currentAmount)) {
            changes[name] = 0;
            return;
        }

        // Determine the original amount based on the percent or tons difference
        let originalAmount;
        if (percentTons === 'percent' && !isNaN(value) && value !== null) {
            originalAmount = reduceIncrease === 'reduce'
                ? currentAmount / (1 - value / 100)
                : currentAmount / (1 + value / 100);
        } else if (percentTons === 'tons' && !isNaN(value) && value !== null) {
            originalAmount = reduceIncrease === 'reduce'
                ? currentAmount + value
                : currentAmount - value;
        } else {
          console.log("ASSUMING ORIGINAL AMOUNT IS CURRENT AMOUNT: ", originalAmount);
            originalAmount = currentAmount; // If no reduction or increase, assume original is current
        }

        // Check if originalAmount is valid; if not, set change to 0
        if (originalAmount == null || isNaN(originalAmount)) {
            changes[name] = 0;
        } else {
            // Calculate the change and store it in the changes object
            changes[name] = currentAmount - originalAmount;
        }
    });
  ...
  public addNewComponent(data: any) {
    ...
    component['changes'] = this.calculateChanges(component);
    ...
    if (this.components.length == 1 || !this.dataIsAddedToForm) {
      document
        .querySelector('.panel-active')?.scrollIntoView({ behavior: 'smooth' });
    } else if (this.components.length > 1) {
      document.querySelector('#review-table')?.scrollIntoView({ behavior: 'smooth' });
    }
